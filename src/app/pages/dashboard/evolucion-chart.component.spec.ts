...
describe('EvolucionChartComponent', () => {
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({ imports: [EvolucionChartComponent] }).compileComponents();
  ...
  it('inicializa con 12 puntos vacios', () => {
    componentRef.setInput('serie', Array.from({ length: 12 }, (_, i) => ({ mes: i + 1, totalCarbonKg: 0 })));
    fixture.detectChanges();
    expect(component.chartData.datasets[0].data).toHaveLength(12);
  });

  it('transforma la serie en datos del grafico', () => {
    const serie: PuntoMensual[] = Array.from({ length: 12 }, (_, i) => ({ mes: i + 1, totalCarbonKg: i < 3 ? 500 : 0 }));
